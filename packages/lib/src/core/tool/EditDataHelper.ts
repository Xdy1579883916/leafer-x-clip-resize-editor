import type { IEditorRotateEvent, IEditorScaleEvent, IEditorSkewEvent, ILayoutBoundsData } from '@leafer-in/interface'
import type { IAlign, IAround, IBoundsData, IPointData, IUI } from '@leafer-ui/interface'
import { AroundHelper, Bounds, BoundsHelper, Direction9, MathHelper, PointHelper } from '@leafer-ui/core'
...
export const EditDataHelper = {
  getScaleData(element: IUI, startBounds: ILayoutBoundsData, direction: Direction9, totalMove: IPointData, lockRatio: boolean | 'corner', around: IAround, flipable: boolean, scaleMode: boolean): IEditorScaleEvent {
    ...
    const useScaleX = scaleX !== 1
    const useScaleY = scaleY !== 1
    ...
    if (dragBounds) {
      ...
      if (!BoundsHelper.includes(allowBounds, localBounds)) {
        ...
        const fitScaleX = realBounds.width / localBounds.width
        const fitScaleY = realBounds.height / localBounds.height
        if (useScaleX)
          scaleX *= fitScaleX
        if (useScaleY)
          scaleY *= fitScaleY // 后续需优化带旋转的场景
      }
    }
